...
restclient::restclient(QObject *parent) : QObject(parent)
{
    ...
    manual_control_request_url = "https://demo.thingsboard.io:443/api/plugins/telemetry/DEVICE/e483c250-5351-11ed-b827-c9be76c6f5d7/values/attributes/CLIENT_SCOPE";
    manual_control_key = "Bearer eyJhbGciOiJIUzUxMiJ9..sViTMWDWZ-vQCP-JoagP7kmSbeOIgA7YkNhhneEuz2lBbmg9Bq0qImKPDFCyGlkcEl8W1oTYuC4bw7FT6rMCng";
...
//    else emit new_manual_control_received(Vx,Vy,Vz,Yawrate,false);
    ...
    if (dt.toMSecsSinceEpoch() - manual_control_json[0].toObject().value("lastUpdateTs").toVariant().toLongLong() < 3000)
    {
        for(int i=0; i< manual_control_json.count(); i++)
        {
            if (manual_control_json[i].toObject().value("key") == "Step1") Step1 = manual_control_json[i].toObject().value("value").toInt();
            else if (manual_control_json[i].toObject().value("key") == "Step2") Step2 = manual_control_json[i].toObject().value("value").toInt();
            else if (manual_control_json[i].toObject().value("key") == "Step3") Step3 = manual_control_json[i].toObject().value("value").toInt();
